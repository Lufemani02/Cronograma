...
exports.obtenerCronogramaDelMes = exports.asignarCronograma = void 0;
const db_1 = __importDefault(require("../config/db"));
const asignarCronograma = async (req, res) => {
    try {
        const { departamento_id, usuario_id, tarea_id, fecha } = req.body;
        // Validación
        if (!departamento_id || !usuario_id || !tarea_id || !fecha) {
            return res.status(400).json({ error: 'Todos los campos son requeridos' });
        }
        // Verifica que el miembro pertenezca al departamento
        const [miembroRows] = await db_1.default.query(`SELECT 1 FROM usuario_departamento 
       WHERE usuario_id = ? AND departamento_id = ?`, [usuario_id, departamento_id]);
        ...
        const [tareaRows] = await db_1.default.query('SELECT 1 FROM tarea WHERE id = ? AND departamento_id = ?', [tarea_id, departamento_id]);
        ...
        const [conflictoRows] = await db_1.default.query('SELECT 1 FROM cronograma WHERE usuario_id = ? AND fecha = ? AND estado != "cancelado"', [usuario_id, fecha]);
        if (conflictoRows.length > 0) {
            return res.status(409).json({ error: 'Este miembro ya está asignado el ' + fecha });
        }
        // Insertar asignación
        const [result] = await db_1.default.query(`INSERT INTO cronograma 
       (departamento_id, usuario_id, tarea_id, fecha, creado_por, estado)
       VALUES (?, ?, ?, ?, ?, 'activo')`, [departamento_id, usuario_id, tarea_id, fecha, req.usuarioId]);
...
exports.asignarCronograma = asignarCronograma;
const obtenerCronogramaDelMes = async (req, res) => {
    try {
...
exports.obtenerCronogramaDelMes = obtenerCronogramaDelMes;
//# sourceMappingURL=cronogramaController.js.map